...
from Bots.piece_values import (pawntable, knightstable, bishopstable, rookstable, 
                          queenstable, kingstable, kings_end_game_table, 
...
class ChessBot:
    def __init__(self):
        ...
        self.opening_moves = [
            ((6, 4), (5, 4)), ((6, 3), (5, 3)), ((7, 1), (5, 2)),
            ((7, 6), (5, 5)), ((6, 2), (5, 2)), ((6, 5), (5, 5))
        ]
        self.move_history = defaultdict(lambda: {'count': 0, 'success': 0})

    def is_square_threatened(self, board: np.ndarray, x: int, y: int, color: str) -> bool:
    ...
    def get_piece_moves(self, board: np.ndarray, x: int, y: int, color: str) -> List[Tuple[int, int]]:
        piece = board[x, y]
        ...
        for x in range(8):
            for y in range(8):
                if board[x, y].startswith('p') and board[x, y][1] == color:
                    pawn_columns[y] += 1
                    if (color == 'w' and x == 1) or (color == 'b' and x == 6):  # Bonus pour les pions près de la promotion
                        score *= 1.2

        for y in range(8):
            if pawn_columns[y] > 0 and (pawn_columns[y-1] > 0 if y > 0 else False or pawn_columns[y+1] > 0 if y < 7 else False):
                score *= 1.1

        return score

    def minimax(self, board: np.ndarray, depth: int, alpha: float, beta: float, maximizing: bool, color: str, start_time: float, time_budget: float) -> Tuple[float, Optional[Tuple[Tuple[int, int], Tuple[int, int]]]]:
...
def chess_bot(player_sequence: List[str], board: np.ndarray, time_budget: float, **kwargs) -> Tuple[Tuple[int, int], Tuple[int, int]]:
    ...
    try:
        while time.time() - start_time < time_budget * 0.9:
            _, current_best_move = bot.minimax(
                board, depth, float('-inf'), float('inf'), True, color, start_time, time_budget
            )
    ...
    if not best_move:
        ...
        for x in range(8):
            for y in range(8):
                if board[x,y] == f"p{color}" and 0 <= x + direction < 8 and board[x+direction,y] == '':
